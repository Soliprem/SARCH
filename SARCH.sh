...
usercheck() {
	! { id -u "$name" >/dev/null 2>&1; } ||
		dialog --colors --title "WARNING!" --yes-label "CONTINUE" --no-label "No wait..." --yesno "The user \`$name\` already exists on this system. SARCH can install for a user already existing, but it will \\Zboverwrite\\Zn any conflicting settings/dotfiles on the user account.\\n\\nSARCH will \\Zbnot\\Zn overwrite your user files, documents, videos, etc., so don't worry about that, but only click <CONTINUE> if you don't mind your settings being overwritten.\\n\\nNote also that SARCH will change $name's password to the one you just gave." 14 70
}
...
newperms() { # Set special sudoers settings for install (or after).
	sed -i "/#SARCH/d" /etc/sudoers
	echo "$* #SARCH" >>/etc/sudoers
}
...
maininstall() { # Installs all needed programs from main repo.
	dialog --title "SARCH Installation" --infobox "Installing \`$1\` ($n of $total). $1 $2" 5 70
	installpkg "$1"
}
...
aurinstall() {
	dialog --title "SARCH Installation" --infobox "Installing \`$1\` ($n of $total) from the AUR. $1 $2" 5 70
	echo "$aurinstalled" | grep -q "^$1$" && return 1
	sudo -u "$name" $aurhelper -S --noconfirm "$1" >/dev/null 2>&1
}

pipinstall() {
	dialog --title "SARCH Installation" --infobox "Installing the Python package \`$1\` ($n of $total). $1 $2" 5 70
...
finalize() {
	...
	dialog --title "All done!" --msgbox "Congrats! Provided there were no hidden errors, the script completed successfully and all the programs and configuration files should be in place.\\n\\nHave fun with your fully functioning arch system.\\n\\n.t Soli" 12 80
}
...
for x in curl ca-certificates base-devel git ntp zsh; do
	dialog --title "SARCH Installation" --infobox "Installing \`$x\` which is required to install and configure other programs." 5 70
	installpkg "$x"
done

dialog --title "SARCH Installation" --infobox "Synchronizing system time to ensure successful and secure installation of software..." 4 70
...
dialog --title "SARCH Installation" --infobox "Finally, installing \`libxft-bgra\` to enable color emoji in suckless software without crashes." 5 70
